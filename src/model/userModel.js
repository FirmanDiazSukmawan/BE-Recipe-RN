...
const verifyUser = (users_id) => {
  return db.query(
    `UPDATE * FROM users SET verif=1 WHERE users.users_id=${users_id}`
  );
};

const createUser = (data) => {
  const { email, phone_number, username, password } = data;
  const role = 0;
  return new Promise((resolve, reject) =>
    db.query(
      `INSERT INTO users(email,phone_number,username, password,role) VALUES('${email}','${phone_number}', '${username}', '${password}', ${role})`,
      (err, res) => {
        if (!err) {
          resolve(res);
        } else {
...
const updateUser = (data, users_id) => {
  ...
  return db.query(
    `UPDATE users SET username='${username}', phone_number='${phone_number}',image='${image}' WHERE users.users_id = ${users_id}`
  );
};
